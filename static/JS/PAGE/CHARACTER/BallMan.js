...
BallMan.prototype.init=function(param,scene)
{
    ...
    this.id=param.id;
    this.state="work";//比如state为ani时用动画控制状态变化，不使用ws同步
    this.index_startpoint=param.index_startpoint;
    ...
    var mesh_head=BABYLON.Mesh.CreateSphere(this.name+"head", 10,  2.0*scal, scene);
    mesh_head.renderingGroupId=2;
    mesh_head.layerMask=2;
    //mesh_head.rotation.x=Math.PI*0.5;
    mesh_head.material=mat_head;
    mesh_head.parent=this.mesh;//这里不使用物理引擎，所以可以把他们绑起来
    ...
    handpoint.position=new BABYLON.Vector3(0,-2.8*scal,6.2*scal);
    this.handpoint=handpoint;
    //左手和右手
    var lefthand=new BABYLON.Mesh(this.name+"lefthand",scene);
    lefthand.parent=this.head;
    lefthand.position=new BABYLON.Vector3(-1*scal,0.2*scal,3.0*scal);
    ...
    righthand.position=new BABYLON.Vector3(1*scal,0.2*scal,3.0*scal);
...
//完整的删除一个card对象的所有内容
BallMan.prototype.dispose=function()
{
    this.state="disposeing";
    this.lab.dispose();
    this.headview.dispose();
    ...
    this.head.dispose();
    this.mesh.dispose();
    if(MyGame.arr_webplayers)
    {
        delete MyGame.arr_webplayers[this.id];
    }
    if(arr_webusers)
    {
        for(var i=0;i<arr_webusers.length;i++)
        {
            if(arr_webusers[i].id==this.id)
            {
                arr_webusers.splice(i,1);
                i--;
            }
        }
    }
}
//以后再在这里加入一个带动画效果的destory方法，动画以dispose结尾